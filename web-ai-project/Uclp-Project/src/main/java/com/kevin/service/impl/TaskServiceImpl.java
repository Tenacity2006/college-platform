...
public class TaskServiceImpl implements TaskService {
    ...
    public void publishTask(Task task,MultipartFile detailFile, Integer userId) {
        ...
        if (detailFile != null && !detailFile.isEmpty()) {
            try {
                detailFilePath = FileUtil.uploadFile(detailFile);
            } catch (IOException e) {
                throw new RuntimeException("任务详情文件上传失败", e);
            }
        }
        ...
        task.setProof(null);
        task.setDetailFile(detailFilePath);
    ...
    public void updateTask(Task task, MultipartFile detailFile, Integer userId) {
        ...
        if (detailFile != null && !detailFile.isEmpty()) {
            try {
                // 调用工具类上传文件
                String filePath = FileUtil.uploadFile(detailFile);
                // 更新数据库中的文件路径
    ...
    public void completeTask(Integer id, Integer userId, MultipartFile proof) {
        ...
        if (proof != null && !proof.isEmpty()) {
            try {
                ...
                // 统一抛出运行时异常，由全局异常处理器捕获
                throw new RuntimeException("凭证文件上传失败", e);
            }
        }
        Task updateParam = new Task();
        updateParam.setId(id);
